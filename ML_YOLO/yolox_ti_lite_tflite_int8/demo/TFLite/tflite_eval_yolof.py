"""
Module: tflite_eval_yolof
This module evaluates a TensorFlow Lite YOLOv model on a COCO format dataset.  It handles preprocessing, inference, postprocessing, and COCO mAP evaluation.
Functions:
  - load_cocoformat_labels: Loads class labels from a COCO annotation file.
  - parse_args: Parses command line arguments.
  - main: The main function that orchestrates the evaluation process.
Classes:
  - CocoFormatDataset: A custom dataset class for loading and preprocessing COCO format data.  Includes methods to manage annotations, load images, and convert predictions to the COCO format for evaluation.
    It also includes a method for generating a per-class mAP table.
"""
...
from utils import COCO_CLASSES, multiclass_nms_class_aware, preprocess, yolofastest_postprocess, easy_preprocess
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--anno_file",
        type=str,
        default="medicine_coco/annotations/medicine_val.json",
...
def main():
    ...
    # setup dataset
    data_list = []
    my_dataset = CocoFormatDataset(data_dir=args.val, anno_file=args.anno_file)
    ...
    for _, (origin_img, _, info_imgs, ids) in enumerate(tqdm(my_dataset)):
        ...
        if args.preprocess_way == "cv2":
            img, _ = easy_preprocess(origin_img, model_img_size)
        else:
            img, _ = preprocess(origin_img, model_img_size)
